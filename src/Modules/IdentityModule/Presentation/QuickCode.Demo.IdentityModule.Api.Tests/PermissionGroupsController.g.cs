...
namespace QuickCode.Demo.IdentityModule.Api.Tests
{
    public class PermissionGroupsControllerTests
    {
        ...
        public async Task ListAsync_Should_Return_Ok_When_Success()
        {
            ...
            var fakeResult = new ListResult<PermissionGroupDto>
            {
                Items = fakeList
            };
            _mediatorMock.Setup(m => m.Send(It.IsAny<ListPermissionGroupQuery>(), default)).ReturnsAsync(new Response<ListResult<PermissionGroupDto>> { Code = 0, Value = fakeResult });
            // Act
        ...
        {
            // Arrange
            _mediatorMock.Setup(m => m.Send(It.IsAny<ListPermissionGroupQuery>(), default)).ReturnsAsync(new Response<ListResult<PermissionGroupDto>> { Code = 1, Message = "Error" });
            // Act
            var result = await _controller.ListAsync(1, 10);
            // Assert
